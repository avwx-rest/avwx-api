...
avwx_api.cache - Module for communicating with the report cache
"""
...
async def get(rtype: str, station: str, force: bool = False) -> {str: object}:
    ...
    for i in range(5):
        try:
            ...
            data = await getattr(db, rtype.lower()).find_one({'_id': station})
...
async def update(rtype: str, data: {str: object}):
    ...
    for i in range(5):
        try:
            db = AsyncIOMotorClient(MONGO_URI).report_cache
            await getattr(db, rtype.lower()).update_one({'_id': id}, {'$set': data}, upsert=True)
            return
        except OperationFailure:
            return
        except AutoReconnect:
